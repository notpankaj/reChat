...
const login = async (model, context) => {
  ...
  const user = await db.user.findOne({
    $or: [{ username: model.username }, { email: model.username }],
  });
  if (!user) {
    log.end();
...
const follow = async (model, context) => {
  ...
  const secondUpdated = await db.user.findOneAndUpdate(
    secondFilter,
    secondUpdate
  );
...
const usersByFilter = async (query, context) => {
  ...
  if (query.gender !== "" && query.gender == undefined) {
    filter = [
      {
        $match: {
          gender: context.user.gender == "male" ? "female" : "male",
    ...
    filter = [
      {
        location: {
          $near: {
            $geometry: { type: "Point", coordinates: [query.lat, query.long] },
            $maxDistance: 10000, // 10 kilometer
          },
        },
      },
      { $limit: pageSize },
  ...
  const blockedUsers = await db.block.find({
    $or: [{ byUser: context.user.id }, { toUser: context.user.id }],
  });
...
// MINE
const userByUsername = async (query, context) => {
  const log = context.logger.start(`services:users:userByUsername `);
  let pageNo = Number(query.pageNo) || 1;
  let pageSize = Number(query.pageSize) || 10;
  let skipCount = pageSize * (pageNo - 1);
