...
const resetPassword = async (req, res) => {
  ...
  try {
    const message = await service.resetPassword(
      req.params.id,
      req.body,
      req.context
    );
...
const uploadProfileImage = async (req, res) => {
  ...
  try {
    const image = await service.uploadProfilePic(
      req.params.id,
      req.files,
      req.context
    );
...
const uploadStory = async (req, res) => {
  ...
  try {
    const story = await service.uploadStory(
      req.params.id,
      req.files,
      req.context
    );
...
const myStatistics = async (req, res) => {
  const log = req.context.logger.start(
    `api:users:myStatistics:${req.params.id}`
  );
  try {
...
const removePicOrVideo = async (req, res) => {
  ...
  try {
    const product = await service.removePicOrVideo(
      req.params.id,
      req.body,
      req.context
    );
...
const userByUsername = async (req, res) => {
  const log = req.context.logger.start(`api:users:userByUsername:${req.query}`);
  try {
    const users = await service.userByUsername(req.query, req.context);
    log.end();
    return response.page(
      res,
      users,
